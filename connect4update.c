//TODO 2: replay will show empty board with each move as it happened in the last game
//TODO 3: use a single linkedlist to record the players moves in a file called slist.c and slist.h
//TODO 4: (cont) A function to insert a new node at the tail will be needed. A function to free the list is also needed. The details of what is stored in each node is up to you.
//TODO 5: use slow() method for replay to slow down replay time
//TODO 6: prompt user again 'q' or 'r'
...
int main() {
    char c;
    printf("Welcome! Press 'q' to quit, 'r' to replay, or any other key to play:\n");
    scanf(" %c", &c);
    while (c != 'q') {
        if (c != 'r') { //nested loop to run PlayConnectFour() if user input is not q and is not r
            PlayConnectFour();
        }
    }
    printf("Bye Bye!\n");
